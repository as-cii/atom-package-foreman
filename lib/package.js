...
export function isPackageInstalled(packageName) {
  return new Promise((resolve, reject) => {
    listPackages().then(installedPackages => {
      resolve(installedPackages.includes(p => packageName));
    }).catch((error) => {
      reject(error);
    });
  });
...
export function removePackageFromManifest(package, manifest = undefined) {
  if (manifest) {
    writePackageManifest(manifest.filter(p => p !== package));
  } else {
    readPackageManifest().then(packageManifest => {
      writePackagedManifest(packageManifest.filter(p => p !== package));
    });
  }
}
